...
/*   Updated: 2024/09/03 19:55:14 by eaboudi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ( ((*node)) && ((*node)->prev && (*node)->prev->type == HERE_DOC) || (((*node)->prev) && (*node)->prev->prev && (*node)->prev->prev->type == HERE_DOC))
	{
...
void	open_heredoc_sy(t_global *global, char *eof)
{
	char 	*read;
	int		len;
		...
		if (tmp->type == HERE_DOC)
		{
			if (tmp->next)
			{
				if (tmp->next->next && tmp->next->next->type == WORD)
					open_heredoc_sy(global, tmp->next->next->content);
			}		
		}
		tmp = tmp->next;
	}
